...
from assessment.service import QuestionService,TranscriptService
    ...
    def post(self,request,pk,id):
        try:
            ...
            if as_queryset.is_valid():
                as_queryset.save()
                answer_audio = as_queryset.data['answer_audio']
                if answer_audio:
                    link = 'https://vidyaai-backend.ultimeet.io/media/audio/392.wav'
                    try :
                        data,text = TranscriptService().get_transcript(upload_url=link)

                        trans = Transcript.objects.create(assessment_transcript=as_queryset,raw_transcript=json.dumps(text),json_transcript=json.dumps(data))
                    except Exception as err:
                        print('ERROR::::',err)
                        code = 'server_error'
                else:
